 ...
 * @param {Function} cb
 */
Connection.prototype.subscribeToStream = function(stream, resolveLinkTos, cb) {
...
Connection.offsets = {};
Connection.offsets.commandOffset = 4;
Connection.offsets.flagsOffset = Connection.offsets.commandOffset + 1;
Connection.offsets.correlationOffset = Connection.offsets.flagsOffset + 1;
Connection.offsets.authOffset = Connection.offsets.correlationOffset + 16;
Connection.offsets.mandatorySize = Connection.offsets.authOffset;
...
Connection.prototype._createTcpPacket = function(command, correlationId, payload) {
  var flags = 0;
  var payloadSize = payload ? payload.length : 0;
  var packetSize = Connection.offsets.mandatorySize + payloadSize;
  var packet = new Buffer(packetSize);
  packet.writeUInt32LE(packetSize - 4, 0);
  packet.writeUInt8(TcpCommands.codeForType[command], Connection.offsets.commandOffset);
  packet.writeUInt8(flags, Connection.offsets.flagsOffset);
  uuid.parse(correlationId, packet, Connection.offsets.correlationOffset);
  if (payloadSize > 0) {
    payload.copy(packet, Connection.offsets.mandatorySize);
  }  
  return packet;
};
...
Connection.prototype._process = function(packet) {
  ...
  var payload = null;
  var size = Connection.offsets.mandatorySize - 4;
  if (packet.length > size) {
    payload = packet.slice(size);
  }
